...
impl AppConfig {
    pub fn new() -> Result<Self, ConfigError> {
        ...
        let auth_database_url = std::env::var("APP_AUTH_DATABASE_URL")
            .or_else(|_| std::env::var("APP_auth_database_url"))
            .or_else(|_| std::env::var("app_auth_database_url"))
            .map_err(|_| ConfigError::Message("Missing APP_AUTH_DATABASE_URL".to_string()))?;

        let core_database_url = std::env::var("APP_CORE_DATABASE_URL")
            .or_else(|_| std::env::var("APP_core_database_url"))
            .or_else(|_| std::env::var("app_core_database_url"))
            .map_err(|_| ConfigError::Message("Missing APP_CORE_DATABASE_URL".to_string()))?;
